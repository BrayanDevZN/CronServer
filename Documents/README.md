...
![Arquitetura visual do Cron Server](assets/cron-server-banner.png)
...
```mermaid
flowchart LR
    Client[Cliente] -->|HTTP + JSON| API[FastAPI API]
    ...
    Cron -->|execute_task.delay| Broker[Celery Broker]
...
Uma descrição completa de cada camada está disponível em [`layer/README.md`](layer/README.md).
...
```bash
curl --request POST http://localhost:8000/requests/ \
...
```bash
curl http://localhost:8000/requests/<instance-token> \
...
```bash
curl --request PATCH http://localhost:8000/requests/ \
  --header "X-instance_token: <instance-token>" \
  --header "Content-Type: application/json" \
...
```bash
curl --request DELETE http://localhost:8000/requests/ \
  --header "X-instance_token: <instance-token>"
```
...
- [Requisitos do sistema](app/requisitos.md)
- [Schema do banco](app/schema.md)
- [Tecnologias](app/Tecnologias.md)
- [Estrutura visual](app/Estrutura.pdf)
- [Fluxo visual](app/Fluxo.pdf)

### Camadas

- [Visão geral das camadas](layer/README.md)
- [Aplicação](layer/application.md)
- [Domínio](layer/domain.md)
- [Infraestrutura](layer/infrastructure.md)
- [Repositório](layer/repository.md)
- [Serviço](layer/service.md)
- [Controller e Docker](layer/controller.md)
- [Logs](layer/logs.md)
- [Testes](layer/tests.md)
...
Construído com FastAPI, Celery, Redis, PostgreSQL e Docker.
